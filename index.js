...
// Fallback to your test secret if the environment variable isn't set yet
const SHARED_SECRET = process.env.SHARED_SECRET || "tK9mXqRwL2pN";
// Fallback to your new test number (formatted with international country code 254)
...
let hasRequestedPairingCode = false; // State gate to prevent duplicate code requests
...
async function startBot() {
  ...
  try {
    ...
    sock = makeWASocket({
      ...
      auth: state,
      browser: Browsers.ubuntu("Chrome"), // FIXED: Using standard 'Chrome' stops WhatsApp from rejecting the link
    ...
    if (!sock.authState.creds.registered && !hasRequestedPairingCode) {
      if (!PHONE_NUMBER) {
        ...
        // 5-second delay to ensure the background websocket connection has entirely stabilized
        setTimeout(async () => {
          try {
            ...
            hasRequestedPairingCode = false; // Allow retry if the core request actually failed
          }
        }, 5000);
      }
    }
    ...
    sock.ev.on("connection.update", (update) => {
      ...
      if (connection === "open") {
        ...
        isConnected = false;
        hasRequestedPairingCode = false; // Reset gate on disconnect so it can clean-request later
