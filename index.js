...
let hasRequestedPairingCode = false; // Prevents double-requesting race conditions
...
async function startBot() {
  ...
  try {
    ...
    sock = makeWASocket({
      ...
      auth: state,
      browser: Browsers.ubuntu("Chrome"), // FIXED: Using 'Chrome' prevents WhatsApp from rejecting the link request
    ...
    if (!sock.authState.creds.registered && !hasRequestedPairingCode) {
      ...
      // Allow the socket connection to stabilize before knocking on WhatsApp's door
      setTimeout(async () => {
        try {
          ...
          hasRequestedPairingCode = false; // Reset to allow retry if it actually failed
        }
      }, 5000); // 5-second window gives Railway enough time to steady the socket stream
    }
    ...
    sock.ev.on("connection.update", (update) => {
      ...
      if (connection === "open") {
        ...
        isConnected = false;
        hasRequestedPairingCode = false; // Reset on close so a fresh connection can try again
